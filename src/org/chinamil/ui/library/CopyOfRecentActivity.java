...
public class CopyOfRecentActivity extends Activity implements OnClickListener {
	...
	private Handler myHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 1:// 下日载进度条最大值
				...
				break;
			case 2:// 删除
				Toast.makeText(CopyOfRecentActivity.this, Heibai.DEL, 1).show();
				break;
			case 0:// 下载 更改进度条
					// 不需要手动控制线程 停止
				/*
				...
				if ((progressBar2.getProgress() < progressBar2.getMax())) {// 是否下载完毕
					...
					Intent intent=new Intent(CopyOfRecentActivity.this,Myservice.class);
			...
			case 5:// pdf下载错误提示
				 ...
				 */
				Toast.makeText(CopyOfRecentActivity.this, Heibai.DOWNERR,
				...
				break;
			case 6:// 下日载进度条最大值x.
				Toast.makeText(CopyOfRecentActivity.this,
    ...
    void createFloatView() {
        ...
        floatView.setBackgroundResource(R.drawable.barcode);
       // powerInfo.setText("二维码" );
        vm =((App)getApplication()).getMywm();
        params = ((App)getApplication()).getMywmParams();
	...
	public void onCreate(Bundle savedInstanceState) {
	   ...
	   /* if (config.orientation == Configuration.ORIENTATION_LANDSCAPE){  //横 
	        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT             
	                );
	     }else if(config.orientation == Configuration.ORIENTATION_PORTRAIT){ //竖
	         setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_USER);
	         
	     }*/
		setContentView(R.layout.main);
		welcom = (ImageView) findViewById(R.id.rl_splash);
		animation = AnimationUtils.loadAnimation(CopyOfRecentActivity.this,
				R.anim.wave_scale);
		...
		if (demo.imageheight==0) {//如果屏幕尺寸不在预计范围内的话
			startActivity(new Intent(CopyOfRecentActivity.this,
		...
		jianbao.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (who.isEmpty()) {
					...
					Toast.makeText(CopyOfRecentActivity.this, Heibai.JWNRUN,
							Toast.LENGTH_SHORT).show();
				}
			}
		});
		...
		manager.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (who.isEmpty()) {
					...
					Toast.makeText(CopyOfRecentActivity.this, Heibai.DOWNRUN,
							Toast.LENGTH_SHORT).show();
				}
		...
		thread2 = new Thread(new Runnable() {
			...
			public void run() { // 请求网络成功的时候
				try {
					...
					Intent intent = new Intent(CopyOfRecentActivity.this,
	...
	private class Mybaseadapterr extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			one.setOnClickListener(CopyOfRecentActivity.this);
			two.setOnClickListener(CopyOfRecentActivity.this);
			three.setOnClickListener(CopyOfRecentActivity.this);
	...
	public void onClick(View v) {
		...
		if (c == 7) {
			startActivityForResult(new Intent(CopyOfRecentActivity.this,
			...
			if (realyList.size() > 0) {// 解析 集合是否有数据
				...
				if (!ffFile.exists()) {// 是否存在
					Toast.makeText(CopyOfRecentActivity.this, Heibai.NODOWN,
							Toast.LENGTH_SHORT).show();
					return;
				} else {
					/*
				...
				if (ffFile.exists()) {
					...
					Toast.makeText(CopyOfRecentActivity.this, Heibai.NODOWN, 1)
							.show();
				}
			...
			Toast.makeText(CopyOfRecentActivity.this, Heibai.UNDONE, 1).show();
		}
		//
	}
	...
	private class Myclickdown implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (realyList.size() > 0) {
				 ...
				 * }
				 */} else { // 不存在网络
				Toast.makeText(CopyOfRecentActivity.this, Heibai.AGAIN,
						Toast.LENGTH_SHORT).show();
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		case R.id.mainmenu_settings:
		    Intent service = new Intent(CopyOfRecentActivity.this,SCANMyService.class);
              stopService(service);
			System.exit(0);
			return true;
	...
	// 关于对话框
	public void about() {
		startActivity(new Intent(CopyOfRecentActivity.this, About.class));
	}
			...
			case 0:
				if (cursor != null) { // 管理功能查询完毕后 有数据跳转
					if (cursor.getCount() > 0) {
						...
						Toast.makeText(CopyOfRecentActivity.this, Heibai.NO, 1)
								.show();
					}
				} else {
					Toast.makeText(CopyOfRecentActivity.this, Heibai.NO, 1).show();
	...
	protected void onResume() {
	   /* Intent service = new Intent(CopyOfRecentActivity.this,
                SCANMyService.class);
        service.putExtra("show", "show");
        startService(service);*/
	    if (!init) {
            createFloatView();
      } if (((App)getApplication()).getAccounts()==0) {//第一次初始化
          vm.addView(floatView, params);
      }
      Log.i("xx", ((App)getApplication()).getAccounts()+"ccccc");
	    super.onResume();
	}
	...
	protected void onPause() {
	 /*   Intent service = new Intent(CopyOfRecentActivity.this,
        ...
        service.putExtra("show", "unshow");
        startService(service);*/
	    ((App)getApplication()).setAccounts(0);
